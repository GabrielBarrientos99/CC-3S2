...
// HU :Historia de Usuario
{
    ...
    [TestClass] // Clase de Codigo de Prueba HU.10
    public class TestRecordingGame
    {
        ...
        public void Init()
        {
            file = "E:/ciclo 2023-1/Desarrollo de Software/Sprints/Sprint5/ReSOSgame/";
            controller = new Controller();            
            controller.Tablero = new Tablero(3); 
            controller.Tablero.InitBoard();
            consola = new Consola(controller.Tablero);
            controller.Juego = new JuegoSimple(controller.Tablero);
            controller.Player1 = new Computer(Tablero.Jugador.JAZUL);
            controller.Player2 = new Computer(Tablero.Jugador.JROJO);
            controller.InitTurn();
        ...
        //Criterio de aceptacion 10.1        
        [TestMethod]
        public void GameIsRecording()
        {
            String archivo =file+"recording_Prueba1.txt";
            // Forzamos que el value del parametro de la funcion sea verdadero
            consola.Recording(true);
            Assert.IsTrue(consola.Grabado);
        }

        //Criterio de aceptacion 10.2        
        [TestMethod]
        public void TextFileCreated()
        {
            String archivo = file + "recording_Prueba2.txt";
            consola.Recording(true);
            controller.CurrentPlayer.MakeMove(0, 0, 0, controller.Juego);
            consola.DisplayBoard();
            consola.WriteBoardToFile(archivo);
            controller.ChangeTurn();
            Assert.IsTrue(File.Exists(archivo));
        }

        //Criterio de aceptacion 10.3        
        [TestMethod]
        public void GameProgressPrintedToFile()
        {
            String archivo = file + "recording_Prueba3.txt";
            consola.Recording(true);
            while (controller.Tablero.EstadoDeJuego == Tablero.GameState.JUGANDO)
            {
                controller.CurrentPlayer.MakeMove(0, 0, 0, controller.Juego);
            ...
            // Verificar si el archivo contiene contenido
            string fileContent = File.ReadAllText(archivo);
            Assert.IsFalse(string.IsNullOrEmpty(fileContent), "El archivo no contiene ningún contenido.");
        }
    }
