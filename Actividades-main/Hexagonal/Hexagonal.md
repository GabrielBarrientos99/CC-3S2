...
**when(repository.highestWordNumber()).thenReturn(HIGHEST_WORD_NUMBER):** configura el comportamiento esperado
del stub "repository" cuando se llama al metodo `highestWordNumber()` y se espera que devuelva el valor de
HIGHEST_WORD_NUMBER = 3

**when(repository.fetchWordByNumber(WORD_NUMBER_SHINE)).thenReturn("SHINE"):** configura el comportamiento esperado
del stub "repository" cuando se llama al metodo `fetchWordByNumber(WORD_NUMBER_SHINE)` para que retorne el 
String "SHINE"

**when(random.next(HIGHEST_WORD_NUMBER)).thenReturn(WORD_NUMBER_SHINE):** configura el comportamiento esperado
del stub random cuando se llama al metodo `next(HIGHEST_WORD_NUMBER)` , se espera que devuelva el valor 
WORD_NUMBER_SHINE. Esto simula la generacion de un numero aleatorio que esta dentro de los limites, es decir
, no supera el "HIGHEST_WORD_NUMBER" = 3.


**var selector = new WordSelection(repository, random) :** Procede a crear una variable selector inicializando
con dos objetos que vendrian a ser estos stubs "repository" y "random"

**String actual = selector.chooseRandomWord():** Crea un objeto de tipo String llamado actual que retorna
el string de un objeto que se encuentra en cierta posición aleatoria.Pero como se ha cambiado por este momento
el comportamiento de los stubs random y repository y este método `chooseRandomWord()` utiliza estos objetos
para generar la palabra random, entonces va salir efectivamente SHINE.

**assertThat(actual).isEqualTo("SHINE"):** Se prueba si el String actual es SHINE en caso sea cierto la prueba
pasará.
...
`WordSelection` : Es una clase que contiene un método(`chooseRandomWord()`) que utiliza las dos funciones que poseen los objetos
`repository` y `random` para poder retornar un String que es la palabra que se encuentra en la posición "x" de la base de datos.
Para esto genera un número aleatorio como máximo el highestWordNumber() de repository.

`WordRepository`: es una interfaz que contiene dos funciones: `fetchWordByNumber( int  )` y `highestWordNumber()`

`chooseRandomWord()`: Función de la clase Word Selection que utiliza las dos funciones que poseen los objetos
`repository` y `random` para poder retornar un String que es la palabra que se encuentra en la posición "x" de la base de datos.
Para esto genera un número aleatorio como máximo el highestWordNumber() de repository.

---

Este código configura el comportamiento de antemano antes que se ejecute las pruebas de los stubs creados
previamente que son repository y random.
...
```
Interfaz o también llamado puerto en el esquema hexagonal que lo que hace es abstraer 
el acceso a datos en el dominio de la aplicación. Proporciona métodos que permiten obtener 
información relacionada con las palabras en el juego o aplicación.
```
package com.wordz.domain; 
